...
series_ciudad = pd.Series(lista_ciudades,
                          index = [
                              "A",
                              "C",
                              "L",
...
ciudad_concat = ciudades_uno.append(
     ciudades_dos,
    verify_integrity = False)
#concat y append son totalmente iguales
...
print(ciudades_uno.head(1))
print(ciudades_uno.tail(1))
...
series_numeros_err = pd.Series(['notiene', '1.0', '2', -3])
#ignore, coerce, raise(default)
print(pd.to_number(series_numeros_err, errors = 'ignore'))
print(pd.to_number(series_numeros_err, errors = 'coerce'))